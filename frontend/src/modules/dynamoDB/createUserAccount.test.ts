...
function allTests() {
	it("creates an account with valid input data", async () => {
		...
		//ASSERT
		expect(response.statusCode).toBe("Your account was created successfully.");
	});
	...
	it("throws an error for an already existing email", async () => {
		...
		//ASSERT
		expect(response.statusCode).toBe("An account already exists for this account.");
	});
	...
	it("throws an error when the email is missing", async () => {
		...
		//ASSERT
		expect(response.statusCode).toBe("Email address is required to create a new account.");
	});
	...
	it("throws an error when the email format is invalid", async () => {
		...
		//ASSERT
		expect(response.statusCode).toBe("The provided email is not in a valid email format.");
	});
	...
	it("throws an error when the password is missing", async () => {
		...
		//ASSERT
		expect(response.statusCode).toBe("A password is required to create an account.");
	});
	...
	it("throws an error for an already existing username", async () => {
		...
		//ASSERT
		expect(response.statusCode).toBe("The provided user name already exists.");
	});
	...
	it("throws an error when the username is missing", async () => {
		...
		//ASSERT
		expect(response.statusCode).toBe("A user name is required to create an account.");
	});
